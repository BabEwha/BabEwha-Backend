...
| --- | --- | --- |
| **배달 모임 생성(리더벗)** | **배달 모임 리스트 나열** | **주문 넣기(참여벗)** |
| 리더벗이 Group 테이블의 모든 정보(제목, 내용, 장소, 최대 인원, 배달 링크, 시간 제한)를 기입하여 모임 생성 | 현재 공구 중인 배달 게시물을 피드의 형태로 나열 | 참여벗이 장바구니 캡쳐본으로 주문을 넣을시 주문 생성 |
